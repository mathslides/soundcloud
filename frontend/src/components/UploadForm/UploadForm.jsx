...
function UploadForm() {
  ...
  const [link, setYoutubeLink] = useState("");
  ...
  const handleSubmit = async (e) => {
    ...
    // Check if either audioFile or link is provided
    if (!audioFile && !link) {
      toast.error('Please upload an audio file or provide a YouTube link.');
      return;
    }
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-2 gap-6">
          <div className="w-full">
            ...
            <input
              ...
              className={`mt-0.5 block w-full rounded-md bg-gray-700 text-white border-gray-600 shadow-sm focus:border-blue-500 focus:ring-blue-500 p-2.5 ${loading ? 'cursor-not-allowed opacity-50 ' : ''}`}
              required
            />
          </div>
          <div className="w-full">
          ...
          <div className="w-full">
            ...
            <select
              ...
              className={`h-1/2 mt-0.5 block w-full rounded-md bg-gray-700 text-white border-gray-600 shadow-sm focus:border-blue-500 focus:ring-blue-500 p-2.5 ${loading ? 'cursor-not-allowed' : ''}`}
              required
            >
          ...
          <div className="w-full">
            <label htmlFor="link" className="block font-medium text-white mt-2">Youtube Link</label>
            <input
              type="text"
              name="link"
              id="link"
          ...
          <div className="w-full">
            ...
            <div className="flex items-center w-full mt-2">
              <label htmlFor="imgUrlInput" className={`flex flex-col items-center justify-center w-2/5 h-36 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-700 text-white ${loading ? 'cursor-not-allowed' : ''}`}>
                ...
                <input
                  ...
                  disabled={loading}
                />
                <button type="button" className="mt-4 text-gray-500 text-sm" onClick={handleOpenModal} disabled={loading}>
