...
function UploadForm() {
  ...
  const genres = useSelector((state) => state.genresReducer.genres); // Assuming you've named your reducer as 'genres'
  console.log("Genres data------------------:", genres);

  useEffect(() => {
    if (loggedInUser?.username) {
      fetchGenres(); // Call the renamed function here
  ...
  const fetchGenres = async () => {
    try {
      await dispatch(getAllGenres()); // Dispatch the action to fetch genres
    } catch (error) {
