...
// import { FaPauseCircle, FaPlayCircle, FaTimes, FaYoutube } from "react-icons/fa";
...
//                   <FaPauseCircle className="w-9 h-9 text-white" onClick={(e) => updateCurrent(e, song)} />
//                 ) : (
//                   <FaPlayCircle className="w-9 h-9 text-white" onClick={(e) => updateCurrent(e, song)} />
//                 ))}
//               </div>
//             )}
...
//                 )}
//               </div>
//               <p className="text-gray-300 truncate-text max-w-full">{song?.artist}</p>
//             </div>
//           </div>
//         ))}
//       </div>
...
//             <button className="absolute right-6 top-6 text-white" onClick={() => setYoutubeLink(null)}>
//               <FaTimes />
//             </button>
//             <iframe
//               width="700"
...
export default function TrendingTracks() {
  ...
  const fetchGenres = async (songId) => {
    if (!songId) return;
    try {
      const response = await dispatch(getOne(songId));
      setSingleSong(response?.currentSong);
  ...
  const updateCurrent = async (e, song) => {
    ...
    if (current && current.id === song?.id) {
      ...
      const fetchedSong = await fetchGenres(song?.id);
      dispatch(setCurrent(fetchedSong || song));
      dispatch(setPlaying(true));
      setShowPlayer(true);
    }
      ...
      <div className="tr-container">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6 p-6">
          {Array(15).fill().map((_, index) => (
            <div key={index} className="bg-gray-900 rounded-lg overflow-hidden shadow-md animate-pulse">
