...
function SongItem({ item }) {
  ...
  const userPlaylists = Array.isArray(databaseSongs)
    ? databaseSongs?.filter((playlist) => playlist?.userId === userId)
    : [];
  const likedSongs = useSelector((state) => state.likedSongs?.likedSongs || []);
  const [singleSong, setSingleSong] = useState(null);

  const [isLiked, setIsLiked] = useState(
    likedSongs.some((song) => song?.Song?.id === item?.id)
  );

  useEffect(() => {
    setIsLiked(
      likedSongs.some(
        (song) => song?.songId === item?.id && song.userId === userId
      )
    );
  }, [likedSongs, item?.id]);
  ...
  const updateCurrent = async () => {
    if (current && current.id === item?.id) {
      ...
      const fetchedSong = await fetchSingleSong(item?.id);
      dispatch(setCurrent(fetchedSong || item));
      dispatch(setPlaying(true));
    }
  };
  ...
  return (
    <div>
      <div className="bg-gray-900 rounded-lg overflow-auto shadow-md transition duration-300 ease-in-out transform hover:-translate-y-1 hover:shadow-lg">
        <div
          className="relative"
        ...
        <div className="p-4">
          <div className="flex flex-col">
            ...
            <div className="flex items-center gap-4 justify-between">
              <div className="truncate-text overflow-hidden overflow-ellipsis whitespace-nowrap max-w-xs">
                {item?.title}
              </div>
              {isLiked ? (
            ...
            <div className="flex flex-col gap-4">
              {userPlaylists.length > 0 ? (
                userPlaylists.map((playlist) => (
                  ...
                  >
                    <h3 className="text-white font-semibold overflow-hidden overflow-ellipsis whitespace-nowrap">
                      {playlist?.name}
                    </h3>
                    <button
